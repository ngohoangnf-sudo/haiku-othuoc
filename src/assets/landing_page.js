...
export function initLandingPage() {
  ...
  class EffectShell {
    ...
    createEventsListeners() {
      this.handleItemMouseOvers = [];
      this.items.forEach((item, index) => {
        const handler = this._onMouseOver.bind(this, index);
        this.handleItemMouseOvers.push([item.element, handler]);
    ...
    destroy() {
      ...
      if (this.handleItemMouseOvers) {
        this.handleItemMouseOvers.forEach(([element, handler]) => {
          element.removeEventListener("mouseover", handler, false);
        });
      }
  ...
  const initialBodyStyle = body.getAttribute("style");
  const initialMainStyle = mainElement ? mainElement.getAttribute("style") : null;
  ...
  class SmoothScroll {
    constructor() {
      ...
      if (!this.DOM.main || !this.DOM.scrollable || !this.DOM.content) {
        return;
      }
    ...
    style() {
      ...
      this.DOM.main.style.zIndex = "1";
    }
    initEvents() {
      // on resize reset the body's height
      this.handleResize = () => this.setSize();
  ...
  preloadImages().then(() => {
    ...
    itemsWrappers.forEach((itemsWrapper) => {
      if (itemsWrapper.querySelector("img")) {
  ...
  return () => {
    ...
    if (initialBodyStyle === null) {
      body.removeAttribute("style");
    } else {
      body.setAttribute("style", initialBodyStyle);
    }

    if (mainElement) {
      if (initialMainStyle === null) {
        mainElement.removeAttribute("style");
      } else {
        mainElement.setAttribute("style", initialMainStyle);
      }
    }
  };
}
