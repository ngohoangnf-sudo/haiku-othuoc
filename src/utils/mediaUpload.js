...
export async function uploadEbookToLibrary(file) {
  if (!(file instanceof File)) {
    throw new Error("File upload không hợp lệ.");
  }

  const presignResponse = await fetch(`${API_BASE}/library/presign`, {
    method: "POST",
    headers: authStore.getAuthHeaders({
      "Content-Type": "application/json",
    }),
  ...
  if (!presignResponse.ok) {
    throw new Error(
      await parseUploadError(presignResponse, "Không lấy được link upload ebook từ máy chủ.")
    );
  }
  ...
  if (!uploadResponse.ok) {
    throw new Error(await parseUploadError(uploadResponse, "Không tải được file ebook lên S3."));
  }
